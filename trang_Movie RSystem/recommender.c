...
int number_users(void* NotUsed,__attribute__((unused)) int argc, char **argv,__attribute__((unused)) char **azColName)
{
...
int findusers(){
	...
	int max = 0;
	sprintf(tmp,"SELECT COUNT(id) FROM Users");
	rc = sqlite3_exec(db, tmp, number_users, &max, &err_msg);
...
void recommender(int uid){
	...
	double* utility_matrix = malloc(sizeof(double) * No_of_users * No_of_movies);
	//char *movienames = (char *)malloc(sizeof(char) * No_of_movies * 1024);
	char *moviegenres = malloc(sizeof(char) * No_of_movies * 1024);

	get_utility_matrix(&utility_matrix); //getting utility matrix which will contain ratings according to user id(row) and movie id(coloumn)
	...
	new_user_movies(newuser, userid);
	...
	for(i=0;i<1000;i++){ //selecting top 10 movies from the recommended movies
		...
		//printf("%d. %s %s",i+1,get_movie_names(recommended_movies[i] * 1024 + 1), &moviegenres[recommended_movies[i] * 1024]);
		if(i>0 && (i+1)%10==0){
